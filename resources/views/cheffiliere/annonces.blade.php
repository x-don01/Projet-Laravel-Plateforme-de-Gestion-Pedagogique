...
@endsection
        ...
        <div class="inline">
            <h1>mes annonces</h1>
            <a class="nvlannonce" href="{{ route('annonces.ChefFiliere_create', ['id' => $id, 'nom' => $nom, 'prenom' => $prenom]) }}" style="color: rgb(255, 255, 255);"><i class="fas fa-pen"></i> Nouvelle Annonce</a>
        </div>
        @isset($annonces)
            ...
            <div style="background-color: rgb(39, 140, 140); width: 50%; padding: 15px; margin: 10px; border-radius: 10px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
                ...
                <div style="display: flex; justify-content: space-between; margin-top: 15px;">
                    <a href="{{ route('cheffiliere.modifierannonces', ['id' => $annonce->id]) }}" style="text-decoration: none;">
                        <button style="background-color: #4CAF50; color: white; padding: 10px; border: none; border-radius: 5px; cursor: pointer;">
                            Modifier
                        </button>
                    </a>
                
                    <form action="{{ route('annonces.supprimerCF', ['annonce' => $annonce->id]) }}" method="post" onsubmit="return confirm('Are you sure you want to delete this announcement?');">
                        @csrf
                        @method('delete')
