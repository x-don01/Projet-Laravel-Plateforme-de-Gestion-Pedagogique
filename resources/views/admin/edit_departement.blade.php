...
<body>
    ...
    <main>
        <section id="home">
        <h1>Modifier Chef d'un departement</h1>
        <form method="POST" action="{{ route('admin.update_departement', ['id' => $departement->id]) }}">

            @csrf
            @method('POST')
        
            <label for="departement">Departement:{{ $departement->departement }}</label>
        
            <label for="nom">Nom:</label>
            <input type="text" name="nom" value="{{ $departement->nom }}" required>
        
            <label for="prenom">Prenom:</label>
            <input type="text" name="prenom" value="{{ $departement->prenom }}" required>
        
            <label for="email">Email:</label>
            <input type="email" name="email" value="{{ $departement->email }}" required>
        
            <label for="password">Password</label>
            <input type="password" name="password" required>

                   <!-- Ajoutez le champ pour "filiere" -->
                   <label for="filiere">Filière ou module:</label>
                   <input type="text" name="filiere" value="{{ $departement->module }}" required>
