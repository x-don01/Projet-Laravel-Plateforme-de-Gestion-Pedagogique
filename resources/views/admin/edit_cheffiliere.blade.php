...
<body>
    ...
    <main>
        <section id="home">
        <h1>Modifier Chef d'une filiere</h1>

                <form method="POST" action="{{ route('admin.update_cheffiliere', ['id' => $cheffiliere->id]) }}">
                    @csrf
                    @method('POST')

                    <label for="filiere">Filiere:{{ $cheffiliere->filiere }}</label>

                    <label for="nom">Nom:</label>
                    <input type="text" name="nom" value="{{ $cheffiliere->nom }}" required>

                    <label for="prenom">Prenom:</label>
                    <input type="text" name="prenom" value="{{ $cheffiliere->prenom }}" required>

                    <label for="email">Email:</label>
                    <input type="email" name="email" value="{{ $cheffiliere->email }}" required>

                    <label for="password">password</label>
                    <input type="password" name="password" required>

                           <!-- Ajoutez le champ pour "filiere" -->
                    <label for="filiere">module ou departement:</label>
                    <input type="text" name="filiere" value="{{ $cheffiliere->module }}" required>
